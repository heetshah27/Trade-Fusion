...
import { BarChart3, BookOpenCheck, Calculator, CircleAlert, FlaskConical, LoaderCircle, LockKeyhole, Plus, RotateCcw, ShieldCheck, Sparkles, Trash2 } from "lucide-react";
...
export default function Backtest() {
  ...
  const checkout = trpc.billing.createCheckout.useMutation({
    ...
    onError: error => { setCheckoutState("idle"); toast.error(error.message); },
  });
  ...
  const startCheckout = () => { setCheckoutState("opening"); checkout.mutate(); };
    ...
    <main className="mx-auto grid min-h-[calc(100vh-8rem)] w-full max-w-5xl place-items-center px-5 py-10 lg:px-8">
      <section className="relative w-full overflow-hidden rounded-3xl border border-violet-300/20 bg-[radial-gradient(circle_at_top_right,rgba(139,92,246,0.20),transparent_38%),linear-gradient(145deg,#101d35,#081221)] p-6 shadow-2xl sm:p-10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div><div className="inline-flex items-center gap-2 rounded-full border border-violet-300/25 bg-violet-400/10 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.16em] text-violet-100"><LockKeyhole className="h-3.5 w-3.5" /> Pro workspace</div><h1 className="mt-5 max-w-2xl text-3xl font-semibold tracking-[-0.05em] text-white sm:text-5xl">Turn historical ideas into a repeatable execution record.</h1><p className="mt-4 max-w-xl text-sm leading-7 text-slate-400">Trade Fusion Pro unlocks the private Backtest workspace: source-backed historical replay, chart drawings, simulated execution, snapshots, and isolated strategy performance.</p><div className="mt-6 flex flex-wrap gap-3"><Button onClick={startCheckout} disabled={checkout.isPending || checkoutState === "opening" || !billing?.billingReady} data-checkout-state={checkoutState} className="tf-press tf-action-glow tf-checkout-cta h-11 rounded-xl bg-blue-500 px-5 text-white shadow-[0_12px_30px_rgba(59,130,246,0.28)] hover:bg-blue-400"><Sparkles className={`mr-2 h-4 w-4 ${checkoutState === "opening" ? "tf-checkout-spinner" : ""}`} />{checkoutState === "opening" ? "Preparing secure checkout…" : checkoutState === "opened" ? "Checkout opened" : "Start 7-day Pro trial"}</Button><a href="/#pricing" className="tf-press inline-flex h-11 items-center rounded-xl border border-white/[0.14] px-5 text-sm font-medium text-slate-200 transition hover:bg-white/[0.06]">Compare plans</a></div>{checkoutState !== "idle" && <p role="status" aria-live="polite" className="tf-checkout-notice mt-4 flex max-w-xl items-center gap-2 text-xs leading-5 text-violet-100"><LoaderCircle className={`h-4 w-4 shrink-0 ${checkoutState === "opening" ? "tf-checkout-spinner" : ""}`} />{checkoutState === "opening" ? "Preparing secure Stripe Checkout…" : "Stripe Checkout opened in a new tab. Complete the secure trial there, then return here for Pro access."}</p>}<p className="mt-3 text-xs text-slate-500">Card required. Then {billing?.product.price ?? "$10 USD"} {billing?.product.cadence ?? "per month"}. Cancel before renewal to avoid the first charge.</p>{billingError && <p className="mt-4 flex items-center gap-2 text-xs text-amber-200"><CircleAlert className="h-4 w-4" />We could not verify billing availability. <button type="button" onClick={() => refetchBilling()} className="underline underline-offset-2">Retry</button></p>}</div>
  ...
  return <div className="tf-backtest-page min-h-full bg-[#07101f] text-white">
    <main className="mx-auto w-full max-w-[1640px] px-5 py-7 lg:px-8 lg:py-9">
      <section className="mb-7 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <Button onClick={() => isReadOnly ? startCheckout() : setSessionOpen(true)} disabled={checkout.isPending || checkoutState === "opening"} data-checkout-state={isReadOnly ? checkoutState : "idle"} className={`tf-press h-10 rounded-xl bg-blue-500 text-white shadow-[0_10px_24px_rgba(59,130,246,0.26)] hover:bg-blue-400 ${isReadOnly ? "tf-action-glow tf-checkout-cta" : ""}`}>{isReadOnly ? <Sparkles className={`mr-2 h-4 w-4 ${checkoutState === "opening" ? "tf-checkout-spinner" : ""}`} /> : <Plus className="mr-2 h-4 w-4" />}{isReadOnly && checkoutState === "opening" ? "Preparing checkout…" : isReadOnly && checkoutState === "opened" ? "Checkout opened" : isReadOnly ? "Renew Pro to edit" : "New strategy session"}</Button>
      </section>
