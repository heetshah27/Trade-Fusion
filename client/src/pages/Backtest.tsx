  ...
  return <div className="min-h-full bg-[#07101f] text-white">
    <main className="mx-auto w-full max-w-[1640px] px-5 py-7 lg:px-8 lg:py-9">
      <section className="mb-7 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <Button onClick={() => isReadOnly ? startCheckout() : setSessionOpen(true)} disabled={checkout.isPending || checkoutState === "opening"} data-checkout-state={isReadOnly ? checkoutState : "idle"} className={`tf-press h-10 rounded-xl bg-sky-400 text-slate-950 shadow-[0_10px_24px_rgba(56,189,248,0.26)] hover:bg-sky-300 ${isReadOnly ? "tf-action-glow tf-checkout-cta" : ""}`}>{isReadOnly ? <Sparkles className={`mr-2 h-4 w-4 ${checkoutState === "opening" ? "tf-checkout-spinner" : ""}`} /> : <Plus className="mr-2 h-4 w-4" />}{isReadOnly && checkoutState === "opening" ? "Preparing checkout…" : isReadOnly && checkoutState === "opened" ? "Checkout opened" : isReadOnly ? "Renew Pro to edit" : "New strategy session"}</Button>
      </section>
