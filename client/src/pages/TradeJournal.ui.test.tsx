...
const mocks = vi.hoisted(() => ({ invalidate: vi.fn(), upsert: vi.fn(), remove: vi.fn() }));
vi.mock("@/lib/trpc", () => ({ trpc: { useUtils: () => ({ tradeJournal: { byTrade: { invalidate: mocks.invalidate }, list: { invalidate: mocks.invalidate } } }), trades: { list: { useQuery: () => ({ data: [{ id: 7, date: "2026-08-16", symbol: "XAUUSD", direction: "LONG", pnl: 5, setupTag: "London Breakout" }] }) } }, tradeJournal: { byTrade: { useQuery: () => ({ data: null, isLoading: false }) }, list: { useQuery: () => ({ data: [] }) }, upsert: { useMutation: () => ({ mutate: mocks.upsert, isPending: false }) }, delete: { useMutation: () => ({ mutate: mocks.remove }) } } } }));
