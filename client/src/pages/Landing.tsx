...
import { ArrowRight, BarChart3, BookOpenCheck, CalendarDays, CheckCircle2, ChevronRight, Cloud, Crosshair, Globe2, Layers3, Loader2, LockKeyhole, Mail, Menu, MessageSquare, ScanLine, Send, ShieldCheck, Sparkles, Target, TrendingUp, UserRound, X } from "lucide-react";
...
function OnboardingLink({ children, className }: { children: React.ReactNode; className: string }) {
  ...
  const beginOnboarding = (event: React.SyntheticEvent<HTMLAnchorElement>) => {
    if (isTransitioning) {
    ...
    try {
      ...
      // Navigation remains functional where browser storage is unavailable.
    }

    event.preventDefault();
    setIsTransitioning(true);
    transitionTimer.current = window.setTimeout(() => setLocation(appRoutes.dashboard), reduceMotion ? 0 : 220);
  };

  const beginOnboardingWithKeyboard = (event: React.KeyboardEvent<HTMLAnchorElement>) => {
  ...
  return (
    <Link
      ...
      onKeyDown={beginOnboardingWithKeyboard}
      aria-busy={isTransitioning || undefined}
      data-onboarding-transition={isTransitioning ? "preparing" : "idle"}
      className={`${className} tf-focus-ring relative overflow-hidden transition-[transform,box-shadow,background-color] duration-200 ease-out ${isTransitioning ? "pointer-events-none" : ""}`}
    >
      <span aria-hidden="true" className={`absolute inset-0 bg-gradient-to-r from-white/[0.16] via-white/[0.04] to-transparent ${reduceMotion ? "" : `transition-transform duration-300 ${isTransitioning ? "translate-x-0" : "-translate-x-full"}`}`} />
      <span className="relative inline-flex items-center justify-center">
        {isTransitioning ? <>Preparing secure sign-in <Loader2 className="ml-2 h-4 w-4 animate-spin" /></> : children}
      </span>
    </Link>
  );
}
...
function MobileWorkspacePreview({ activeTab, onSelect }: { activeTab: PreviewTab; onSelect: (tab: PreviewTab) => void }) {
  ...
  return (
    <section data-testid="native-mobile-workspace-preview" data-layout="native-card" className="tf-native-mobile-preview mx-auto w-full max-w-[22rem] px-3 md:hidden">
      <div className="overflow-hidden rounded-[1.6rem] border border-blue-200/[0.14] bg-[#0a0d12]/95 p-3 shadow-[0_24px_56px_rgba(0,0,0,0.48)] backdrop-blur-xl">
        ...
        <div className="mt-3 min-h-[205px] rounded-xl border border-white/[0.07] bg-[#07090d] p-3">
          ...
          {activeTab === "backtest" && <div className="space-y-3"><div className="flex items-start justify-between"><div><p className="font-mono text-[8px] uppercase tracking-[0.17em] text-blue-300">Private replay</p><h3 className="mt-1 text-base font-semibold text-white">XAU/USD · 15m</h3></div><span className="rounded-full bg-blue-400/[0.10] px-2 py-1 font-mono text-[8px] text-blue-200">SIMULATED</span></div><div data-testid="mobile-execution-chart" data-chart-style="candlestick-execution" className="relative h-24 overflow-hidden rounded-lg border border-white/[0.07] bg-[#05070b]"><div className="absolute inset-0 opacity-25 [background-image:linear-gradient(#315a8b_1px,transparent_1px),linear-gradient(90deg,#315a8b_1px,transparent_1px)] [background-size:24px_20px]" /><svg viewBox="0 0 400 120" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-label="Original simulated candlestick execution chart"><rect x="170" y="29" width="125" height="54" fill="#60a5fa" fillOpacity="0.12" stroke="#93c5fd" strokeOpacity="0.55" /><rect x="235" y="16" width="72" height="26" fill="#34d399" fillOpacity="0.12" stroke="#6ee7b7" strokeOpacity="0.52" /><rect x="235" y="42" width="72" height="25" fill="#fb7185" fillOpacity="0.11" stroke="#fda4af" strokeOpacity="0.46" /><g strokeWidth="1.6" stroke="#b6d0ea"><path d="M34 96V64M34 73V88M62 90V54M62 60V78M90 81V41M90 49V69M118 96V59M118 66V86M146 80V43M146 50V72M174 88V48M174 57V80M202 75V32M202 41V62M230 62V22M230 31V53M258 68V28M258 36V58M286 50V14M286 23V43M314 42V8M314 16V34M342 50V20M342 28V43" /></g><g fill="#2dd4bf"><rect x="29" y="73" width="10" height="15" rx="1"/><rect x="85" y="49" width="10" height="20" rx="1"/><rect x="141" y="50" width="10" height="22" rx="1"/><rect x="197" y="41" width="10" height="21" rx="1"/><rect x="225" y="31" width="10" height="22" rx="1"/><rect x="281" y="23" width="10" height="20" rx="1"/><rect x="309" y="16" width="10" height="18" rx="1"/></g><g fill="#fb7185"><rect x="57" y="60" width="10" height="18" rx="1"/><rect x="113" y="66" width="10" height="20" rx="1"/><rect x="169" y="57" width="10" height="23" rx="1"/><rect x="253" y="36" width="10" height="22" rx="1"/><rect x="337" y="28" width="10" height="15" rx="1"/></g><line x1="0" x2="400" y1="53" y2="53" stroke="#60a5fa" strokeWidth="1" strokeDasharray="4 4" opacity="0.82"/><line x1="0" x2="400" y1="42" y2="42" stroke="#34d399" strokeWidth="1" strokeDasharray="4 4" opacity="0.72"/></svg><span className="absolute left-[43%] top-[42%] font-mono text-[7px] text-blue-200">RANGE</span><span className="absolute right-2 top-[31%] font-mono text-[7px] text-emerald-300">TP</span><span className="absolute right-2 top-[52%] font-mono text-[7px] text-rose-300">SL</span></div><div className="grid grid-cols-3 gap-2"><div className="rounded-lg bg-white/[0.025] p-2"><p className="font-mono text-[7px] text-slate-500">ENTRY</p><p className="mt-1 font-mono text-[9px] text-white">2,381.20</p></div><div className="rounded-lg bg-white/[0.025] p-2"><p className="font-mono text-[7px] text-slate-500">R:R</p><p className="mt-1 font-mono text-[9px] text-emerald-300">1:2.4</p></div><div className="rounded-lg bg-blue-500/[0.10] p-2"><p className="font-mono text-[7px] text-blue-300">PLAN</p><p className="mt-1 font-mono text-[9px] text-blue-100">LONG</p></div></div></div>}
...
function WorkspacePreview() {
  ...
  return (
    ...
    >
      ...
      <div className="tf-laptop-stage relative" data-testid="workspace-preview-laptop" data-tilt-interactive="desktop-only" data-hover-glow="fine-pointer" data-hover-pulse={reducedMotion ? "disabled" : "fine-pointer"} data-hover-motion={reducedMotion ? "disabled" : "enabled"} data-mobile-preview="desktop-laptop" onPointerMove={updateLaptopTilt} onPointerLeave={resetLaptopTilt}>
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_155px]">
                    <div className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#071225]">
                      ...
                      <div data-testid="backtest-execution-chart" data-chart-style="candlestick-execution" className="relative h-[205px] overflow-hidden bg-[#05070b] sm:h-[250px]">
                        <div className="pointer-events-none absolute inset-0 opacity-35 [background-image:linear-gradient(#315a8b_1px,transparent_1px),linear-gradient(90deg,#315a8b_1px,transparent_1px)] [background-size:48px_38px]" />
                        <div className="pointer-events-none absolute bottom-0 left-0 right-0 h-12 border-t border-white/[0.06] bg-[#070a10]/88" />
                        <svg viewBox="0 0 760 360" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-label="Original simulated XAU/USD candlestick execution chart">
                          <rect x="280" y="98" width="230" height="152" fill="#60a5fa" fillOpacity="0.11" stroke="#93c5fd" strokeOpacity="0.62" strokeWidth="1.2" />
                          <rect x="430" y="43" width="168" height="62" fill="#34d399" fillOpacity="0.13" stroke="#6ee7b7" strokeOpacity="0.66" strokeWidth="1.2" />
                          <rect x="430" y="105" width="168" height="68" fill="#fb7185" fillOpacity="0.11" stroke="#fda4af" strokeOpacity="0.55" strokeWidth="1.2" />
                          <g stroke="#b6d0ea" strokeWidth="2.1" opacity="0.94"><path d="M34 266V196M34 214V246M76 249V176M76 190V232M118 236V144M118 158V220M160 263V175M160 188V245M202 241V157M202 170V226M244 228V126M244 142V210M286 252V164M286 180V238M328 224V112M328 130V204M370 202V80M370 96V184M412 217V102M412 121V198M454 184V56M454 73V165M496 155V38M496 55V137M538 169V64M538 82V151M580 127V24M580 42V109M622 104V8M622 25V86M664 127V42M664 59V108M706 88V0M706 17V71" /></g>
                          <g fill="#2dd4bf"><rect x="27" y="214" width="14" height="32" rx="1"/><rect x="111" y="158" width="14" height="62" rx="1"/><rect x="195" y="170" width="14" height="56" rx="1"/><rect x="237" y="142" width="14" height="68" rx="1"/><rect x="321" y="130" width="14" height="74" rx="1"/><rect x="363" y="96" width="14" height="88" rx="1"/><rect x="447" y="73" width="14" height="92" rx="1"/><rect x="489" y="55" width="14" height="82" rx="1"/><rect x="573" y="42" width="14" height="67" rx="1"/><rect x="615" y="25" width="14" height="61" rx="1"/><rect x="699" y="17" width="14" height="54" rx="1"/></g>
                          <g fill="#fb7185"><rect x="69" y="190" width="14" height="42" rx="1"/><rect x="153" y="188" width="14" height="57" rx="1"/><rect x="279" y="180" width="14" height="58" rx="1"/><rect x="405" y="121" width="14" height="77" rx="1"/><rect x="531" y="82" width="14" height="69" rx="1"/><rect x="657" y="59" width="14" height="49" rx="1"/></g>
                          <line x1="0" y1="165" x2="760" y2="165" stroke="#60a5fa" strokeWidth="1.2" strokeDasharray="6 6" opacity="0.9"/><line x1="0" y1="105" x2="760" y2="105" stroke="#34d399" strokeWidth="1.2" strokeDasharray="6 6" opacity="0.82"/><line x1="0" y1="173" x2="760" y2="173" stroke="#fb7185" strokeWidth="1.2" strokeDasharray="6 6" opacity="0.66"/>
                          <g opacity="0.72"><rect x="16" y="300" width="16" height="23" fill="#34d399"/><rect x="58" y="289" width="16" height="34" fill="#fb7185"/><rect x="100" y="297" width="16" height="26" fill="#34d399"/><rect x="142" y="281" width="16" height="42" fill="#34d399"/><rect x="184" y="294" width="16" height="29" fill="#fb7185"/><rect x="226" y="276" width="16" height="47" fill="#34d399"/><rect x="268" y="288" width="16" height="35" fill="#fb7185"/><rect x="310" y="271" width="16" height="52" fill="#34d399"/><rect x="352" y="280" width="16" height="43" fill="#34d399"/><rect x="394" y="291" width="16" height="32" fill="#fb7185"/><rect x="436" y="265" width="16" height="58" fill="#34d399"/><rect x="478" y="274" width="16" height="49" fill="#34d399"/><rect x="520" y="286" width="16" height="37" fill="#fb7185"/><rect x="562" y="259" width="16" height="64" fill="#34d399"/><rect x="604" y="270" width="16" height="53" fill="#34d399"/><rect x="646" y="281" width="16" height="42" fill="#fb7185"/><rect x="688" y="257" width="16" height="66" fill="#34d399"/></g>
                        </svg>
                        <div className="absolute left-[37%] top-[29%] rounded border border-blue-300/30 bg-[#071225]/80 px-1.5 py-0.5 font-mono text-[7px] text-blue-100">ACCUMULATION RANGE</div>
                        <div className="absolute right-2 top-[23%] rounded bg-emerald-400/10 px-1.5 py-0.5 font-mono text-[8px] text-emerald-300">TP 2,402.80</div>
                        <div className="absolute right-2 top-[47%] rounded bg-rose-400/10 px-1.5 py-0.5 font-mono text-[8px] text-rose-300">SL 2,372.40</div>
                        <div className="absolute bottom-[19%] left-3 rounded-lg border border-blue-300/25 bg-[#071225]/92 px-2 py-1 font-mono text-[8px] text-blue-100">LONG 2,381.20 · 1:2.4</div>
                        <div className="absolute right-3 top-3 rounded-lg border border-blue-300/20 bg-[#071225]/90 px-2 py-1 font-mono text-[8px] text-slate-300">Replay 58%</div>
                      </div>
                    </div>
...
export default function Landing() {
  ...
  return (
    <main data-mobile-palette="near-black" className="min-h-screen overflow-hidden bg-[#050609] text-white">
      ...
      <header className="relative z-20 mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          <OnboardingLink className="tf-cta-primary !bg-[oklch(0.66_0.18_250)] px-4 py-2.5 text-sm inline-flex !shadow-blue-500/20 hover:!bg-[oklch(0.72_0.15_250)]">Get started <ArrowRight className="ml-2 h-4 w-4" /></OnboardingLink>
        </div>
      ...
      {mobileOpen && (
        <div className="relative z-20 mx-5 rounded-2xl border border-white/[0.09] bg-[#0c1a31]/95 p-5 shadow-2xl backdrop-blur-xl md:hidden">
          <div className="grid gap-3 text-sm text-slate-300">
            ...
            <OnboardingLink className="mt-2 inline-flex items-center justify-center rounded-xl bg-[oklch(0.66_0.18_250)] px-4 py-3 font-semibold text-white">Get started <ArrowRight className="ml-2 h-4 w-4" /></OnboardingLink>
          </div>
        </div>
      )}
      ...
      <section id="start" data-testid="cinematic-hero" data-depth-interactive="desktop-only" onPointerMove={updateHeroDepth} onPointerLeave={resetHeroDepth} className="tf-cinematic-hero relative z-10 mx-auto max-w-7xl px-5 pb-24 pt-20 text-center sm:pb-32 sm:pt-28 lg:px-8 lg:pt-32">
        <div aria-hidden="true" data-testid="hero-3d-scene" data-motion={reducedMotion ? "disabled" : "enabled"} data-glow-style="edge-light" className="tf-3d-hero-scene pointer-events-none absolute inset-0 overflow-hidden">
          <span className="tf-3d-hero-ambient-glow" />
        ...
        <div className="tf-rise tf-rise-delay-3 mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <OnboardingLink className="tf-cta-primary !bg-[oklch(0.66_0.18_250)] h-12 px-7 text-base font-semibold shadow-xl !shadow-blue-500/20 hover:!bg-[oklch(0.72_0.15_250)]">Get started free <ArrowRight className="ml-2 h-4 w-4" /></OnboardingLink>
      ...
      <section id="platform" className="relative z-10 border-y border-white/[0.08] bg-[#050609] py-24 sm:py-32">
      ...
      <section id="workflow" className="relative z-10 border-b border-white/[0.08] bg-[#071327] py-20 sm:py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          <div className="flex flex-col justify-between gap-7 rounded-[2rem] border border-blue-300/[0.15] bg-[linear-gradient(125deg,rgba(20,55,109,0.42),rgba(5,18,38,0.78))] p-7 sm:flex-row sm:items-center sm:p-10">
            ...
            <OnboardingLink className="tf-cta-primary !bg-[oklch(0.66_0.18_250)] h-12 shrink-0 px-6 !shadow-blue-500/20 hover:!bg-[oklch(0.72_0.15_250)]">Build your workspace <ArrowRight className="ml-2 h-4 w-4" /></OnboardingLink>
          </div>
        </div>
      </section>
        ...
        <div className="relative mx-auto max-w-6xl">
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-2">
            <article className="tf-hover-lift rounded-[1.75rem] border border-white/[0.10] bg-[linear-gradient(150deg,rgba(17,35,62,0.92),rgba(6,15,29,0.92))] p-6 sm:p-8">
              ...
              <OnboardingLink className="tf-press mt-8 inline-flex h-11 items-center rounded-xl border border-white/[0.14] px-5 text-sm font-semibold text-slate-100 transition hover:bg-white/[0.06]">Get started free <ArrowRight className="ml-2 h-4 w-4" /></OnboardingLink>
            </article>
      ...
      <section id="security" className="relative z-10 px-5 pb-24 sm:pb-32 lg:px-8">
        <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-blue-300/[0.20] bg-[radial-gradient(circle_at_20%_0%,oklch(0.55_0.16_250_/_0.30),transparent_36rem),linear-gradient(135deg,#122b56,#0a1a35_56%,#061226)] p-8 sm:p-14 shadow-2xl">
          <div className="max-w-2xl">
            ...
            <OnboardingLink className="mt-9 inline-flex h-12 items-center rounded-xl bg-white px-7 text-sm font-semibold text-[#0a1830] transition hover:bg-blue-50 shadow-xl">
