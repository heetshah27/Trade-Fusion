...
export function BacktestReplay({ session }: { session: ReplaySession }) {
  ...
  const executionPreview = useMemo(() => {
    ...
    const gross = executionDirection === "LONG" ? (exit - entry) * quantity : (entry - exit) * quantity;
    const risk = Number(executionRisk);
    const riskAmount = Number.isFinite(risk) ? Math.abs(entry - risk) * quantity : null;
    return { gross, net: gross - fees, riskAmount, rMultiple: riskAmount && riskAmount > 0 ? (gross - fees) / riskAmount : null };
  }, [executionDirection, executionEntry, executionExit, executionQuantity, executionFees, executionRisk]);
  ...
  useEffect(() => {
    ...
    if (drawingTool !== "none") {
      chart.subscribeClick(param => {
        ...
        if (!pendingAnchor) { setPendingAnchor(anchor); return; }
        const zoneFloor = Math.min(pendingAnchor.price, anchor.price);
        const zoneCeiling = Math.max(pendingAnchor.price, anchor.price);
        createAnnotation.mutate({
          sessionId: session.id,
          kind: drawingTool,
          price: drawingTool === "zone" ? zoneFloor : pendingAnchor.price,
          endPrice: drawingTool === "zone" ? zoneCeiling : anchor.price,
          startAt: timestampToIso(pendingAnchor.time),
          endAt: timestampToIso(anchor.time),
          label: drawingTool === "zone" ? "Supply / demand zone" : "Trendline",
        });
      });
    }
