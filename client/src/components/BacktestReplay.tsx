...
export function BacktestReplay({ session }: { session: ReplaySession }) {
  ...
  const sourceNeedsDaily = symbol === "XAUUSD";
  ...
  return <section className="mt-5 overflow-hidden rounded-2xl border border-violet-300/[0.16] bg-[#0c1830]/88">
    <div className="flex flex-col gap-4 border-b border-violet-300/[0.10] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><select aria-label="Replay symbol" value={symbol} onChange={event => { const next = event.target.value as (typeof pairs)[number]; setSymbol(next); if (next === "XAUUSD") setInterval("1d"); else if (["EURUSD", "GBPUSD", "USDJPY"].includes(next) && !["15m", "1h", "1d"].includes(interval)) setInterval("1h"); }} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200">{pairs.map(pair => <option key={pair} value={pair}>{pair === "XAUUSD" ? "XAU/USD · daily price" : pair.replace("USD", "/USD")}</option>)}</select><select aria-label="Replay interval" value={interval} onChange={event => setInterval(event.target.value as ReplayInterval)} disabled={sourceNeedsDaily} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200 disabled:cursor-not-allowed disabled:opacity-50">{(sourceNeedsDaily ? ["1d"] : (["EURUSD", "GBPUSD", "USDJPY"].includes(symbol) ? ["15m", "1h", "1d"] : intervals)).map(value => <option key={value} value={value}>{value}</option>)}</select><select aria-label="Replay date range" value={rangeDays ?? "all"} onChange={event => setRangeDays(event.target.value === "all" ? null : Number(event.target.value))} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200"><option value="all">All loaded</option><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option></select></div>
    </div>
