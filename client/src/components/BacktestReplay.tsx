...
export function BacktestReplay({ session }: { session: ReplaySession }) {
  ...
  const sourceNeedsDaily = symbol === "EURUSD" || symbol === "GBPUSD" || symbol === "USDJPY" || symbol === "XAUUSD";

  useEffect(() => {
    if (sourceNeedsDaily && interval !== "1d") setInterval("1d");
  }, [sourceNeedsDaily, interval]);

  useEffect(() => {
    setCursor(Math.max(0, chartPoints.length - 1));
    setPlaying(false);
  }, [symbol, interval, chartPoints.length]);

  useEffect(() => {
    if (!playing || cursor >= chartPoints.length - 1) { if (cursor >= chartPoints.length - 1) setPlaying(false); return; }
    const timer = window.setInterval(() => setCursor(current => Math.min(current + 1, chartPoints.length - 1)), 650);
    return () => window.clearInterval(timer);
  }, [playing, cursor, chartPoints.length]);

  const visibleCandles = useMemo(() => candles.slice(0, cursor + 1), [candles, cursor]);
  const visiblePrices = useMemo(() => prices.slice(0, cursor + 1), [prices, cursor]);
  const visibleTimes = data?.seriesType === "line" ? visiblePrices : visibleCandles;
  const visibleTrades = useMemo(() => session.trades.filter(trade => markerTimeForDate(visibleTimes, trade.date) !== null), [session.trades, visibleTimes]);
  ...
  useEffect(() => {
    ...
    const markers = visibleTrades.flatMap(trade => {
      const time = markerTimeForDate(visibleTimes, trade.date);
      if (!time) return [];
  ...
  }, [data?.seriesType, visibleCandles, visiblePrices, visibleTimes, visibleTrades]);

  const current = chartPoints[cursor];
  return <section className="mt-5 overflow-hidden rounded-2xl border border-violet-300/[0.16] bg-[#0c1830]/88">
    <div className="flex flex-col gap-4 border-b border-violet-300/[0.10] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><select aria-label="Replay symbol" value={symbol} onChange={event => { const next = event.target.value as (typeof pairs)[number]; setSymbol(next); if (["EURUSD", "GBPUSD", "USDJPY", "XAUUSD"].includes(next)) setInterval("1d"); }} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200">{pairs.map(pair => <option key={pair} value={pair}>{pair === "XAUUSD" ? "XAU/USD · daily price" : pair.replace("USD", "/USD")}</option>)}</select><select aria-label="Replay interval" value={interval} onChange={event => setInterval(event.target.value as ReplayInterval)} disabled={sourceNeedsDaily} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200 disabled:cursor-not-allowed disabled:opacity-50">{(sourceNeedsDaily ? ["1d"] : intervals).map(value => <option key={value} value={value}>{value}</option>)}</select></div>
    </div>
    {!initialSymbol && <div className="border-b border-amber-300/[0.12] bg-amber-400/[0.06] px-5 py-3 text-xs text-amber-100">This Backtest session uses <strong>{session.symbol}</strong>. Choose a supported replay symbol to compare your saved simulation against source-backed history. Crypto uses Kraken OHLC; EUR/USD, GBP/USD, USD/JPY, and XAU/USD use licensed Alpha Vantage daily data.</div>}
    {data?.sourceStatus === "unavailable" ? <div className="grid min-h-[385px] place-items-center p-8 text-center"><div><p className="text-sm font-medium text-slate-200">Historical source data is unavailable right now</p><p className="mt-2 max-w-md text-xs leading-5 text-slate-500">No fallback candles or prices are fabricated. Try another supported market or return when the provider responds.</p></div></div> : !chartPoints.length || isFetching ? <div className="grid min-h-[385px] place-items-center text-sm text-slate-500">Loading source-backed {data?.seriesType === "line" ? "price history" : "candles"}…</div> : <><div ref={chartContainer} data-testid="historical-replay-chart" className="min-h-[385px] w-full" /><div className="flex flex-col gap-3 border-t border-violet-300/[0.10] bg-black/[0.10] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="font-mono text-[10px] uppercase tracking-[0.13em] text-slate-500">Replay {data?.seriesType === "line" ? "price" : "candle"} · <span className="text-violet-200">{current ? replayDateLabel(current.time) : "—"}</span> · {cursor + 1}/{chartPoints.length}</div><div className="flex items-center gap-2"><Button variant="outline" size="sm" aria-label="Previous candle" onClick={() => { setPlaying(false); setCursor(value => Math.max(0, value - 1)); }} disabled={cursor === 0} className="border-blue-200/[0.12] bg-transparent text-slate-300"><ChevronLeft className="h-4 w-4" /></Button><Button size="sm" aria-label={playing ? "Pause replay" : "Play replay"} onClick={() => setPlaying(value => !value)} className="bg-violet-500 hover:bg-violet-400">{playing ? <CirclePause className="mr-1.5 h-4 w-4" /> : <CirclePlay className="mr-1.5 h-4 w-4" />}{playing ? "Pause" : "Play"}</Button><Button variant="outline" size="sm" aria-label="Next candle" onClick={() => { setPlaying(false); setCursor(value => Math.min(chartPoints.length - 1, value + 1)); }} disabled={cursor >= chartPoints.length - 1} className="border-blue-200/[0.12] bg-transparent text-slate-300"><ChevronRight className="h-4 w-4" /></Button><Button variant="outline" size="sm" onClick={() => { setPlaying(false); setCursor(0); }} className="border-blue-200/[0.12] bg-transparent text-slate-300"><StepForward className="mr-1.5 h-3.5 w-3.5" /> Restart</Button></div></div></>}
  </section>;
}
