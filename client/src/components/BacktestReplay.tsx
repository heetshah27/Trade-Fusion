...
import { ChevronLeft, ChevronRight, CirclePause, CirclePlay, Layers3, Plus, StepForward, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { trpc } from "@/lib/trpc";
import { filterReplayRange, replayDateLabel, toReplayInterval, toReplaySymbol, type ReplayInterval } from "@/lib/replay";

type ReplayTrade = { id: number; date: string; entryAt: string | null; exitAt: string | null; direction: "LONG" | "SHORT"; entryPrice: number; exitPrice: number; pnl: number; fees: number };
type ReplaySession = { id: number; symbol: string; timeframe: string; trades: ReplayTrade[] };
type ReplayAnnotation = { id: number; sessionId: number; kind: "support" | "resistance"; price: number; label: string; createdAt: string };
...
export function BacktestReplay({ session }: { session: ReplaySession }) {
  ...
  const [annotationKind, setAnnotationKind] = useState<ReplayAnnotation["kind"]>("support");
  ...
  const createAnnotation = trpc.backtest.createAnnotation.useMutation({ onSuccess: () => { annotationsQuery.refetch(); setAnnotationLabel(""); } });
  ...
  useEffect(() => {
    ...
    const markers = visibleTrades.flatMap(trade => {
      ...
      if (entryTime) tradeMarkers.push({ time: entryTime as UTCTimestamp, position: entryPosition as "belowBar" | "aboveBar", color: trade.direction === "LONG" ? "#56d39b" : "#fb7185", shape: trade.direction === "LONG" ? "arrowUp" : "arrowDown", text: `${trade.direction} entry` });
      if (exitTime) tradeMarkers.push({ time: exitTime as UTCTimestamp, position: exitPosition as "belowBar" | "aboveBar", color: positive ? "#56d39b" : "#fb7185", shape: "circle", text: `Exit ${positive ? "+" : ""}${(trade.pnl - trade.fees).toFixed(2)}` });
      return tradeMarkers;
    });
    ...
    annotations.forEach(annotation => {
      series.createPriceLine({
        ...
        color: annotation.kind === "support" ? "#38bdf8" : "#fb7185",
        lineWidth: 2,
        axisLabelVisible: true,
        title: annotation.label || (annotation.kind === "support" ? "Support" : "Resistance"),
      });
    });
  ...
  }, [annotations, data?.seriesType, visibleCandles, visiblePrices, visibleTimes, visibleTrades]);
  ...
  return <section className="mt-5 overflow-hidden rounded-2xl border border-violet-300/[0.16] bg-[#0c1830]/88">
    <div className="flex flex-col gap-4 border-b border-violet-300/[0.10] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><select aria-label="Replay symbol" value={symbol} onChange={event => { const next = event.target.value as (typeof pairs)[number]; setSymbol(next); if (next === "XAUUSD" && !goldIntervals.includes(interval)) setInterval("1h"); else if (["EURUSD", "GBPUSD", "USDJPY"].includes(next) && !["15m", "1h", "1d"].includes(interval)) setInterval("1h"); }} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200">{pairs.map(pair => <option key={pair} value={pair}>{pair === "XAUUSD" ? "XAU/USD · intraday candles" : pair.replace("USD", "/USD")}</option>)}</select><select aria-label="Replay interval" value={interval} onChange={event => setInterval(event.target.value as ReplayInterval)} disabled={sourceNeedsDaily} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200 disabled:cursor-not-allowed disabled:opacity-50">{((symbol === "XAUUSD" ? goldIntervals : (["EURUSD", "GBPUSD", "USDJPY"].includes(symbol) ? ["15m", "1h", "1d"] : intervals))).map(value => <option key={value} value={value}>{value}</option>)}</select><select aria-label="Replay date range" value={rangeDays ?? "all"} onChange={event => setRangeDays(event.target.value === "all" ? null : Number(event.target.value))} className="h-9 rounded-lg border border-blue-200/[0.12] bg-white/[0.04] px-2 text-xs text-slate-200"><option value="all">All loaded</option><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option></select></div>
    </div>
    {!initialSymbol && <div className="border-b border-amber-300/[0.12] bg-amber-400/[0.06] px-5 py-3 text-xs text-amber-100">This Backtest session uses <strong>{session.symbol}</strong>. Choose a supported replay symbol to compare your saved simulation against source-backed history. Crypto uses Kraken OHLC; EUR/USD, GBP/USD, USD/JPY, and XAU/USD use licensed provider data.</div>}
    {data?.sourceStatus === "unavailable" ? <div className="grid min-h-[385px] place-items-center p-8 text-center"><div><p className="text-sm font-medium text-slate-200">Historical source data is unavailable right now</p><p className="mt-2 max-w-md text-xs leading-5 text-slate-500">No fallback candles or prices are fabricated. Try another supported market or return when the provider responds.</p></div></div> : !chartPoints.length || isFetching ? <div className="grid min-h-[385px] place-items-center text-sm text-slate-500">Loading source-backed {data?.seriesType === "line" ? "price history" : "candles"}…</div> : <><div className="flex flex-wrap items-center gap-2 border-b border-violet-300/[0.08] px-5 py-2 text-[10px] uppercase tracking-[0.12em] text-slate-500"><span className="text-emerald-200">▲ Simulated entry</span><span className="text-rose-200">● Simulated exit</span><span>Markers appear when a saved simulated trade enters the selected replay range.</span></div><div ref={chartContainer} data-testid="historical-replay-chart" className="min-h-[385px] w-full" /><div className="border-t border-violet-300/[0.10] bg-violet-400/[0.04] px-5 py-4"><div className="mb-2 flex items-center justify-between"><div><p className="text-xs font-semibold text-slate-200">Chart annotations</p><p className="text-[11px] text-slate-500">Add private horizontal support or resistance levels for this Backtest session.</p></div><span className="font-mono text-[9px] uppercase tracking-[0.12em] text-violet-200">Private to this session</span></div><div className="flex flex-col gap-2 lg:flex-row"><select aria-label="Annotation kind" value={annotationKind} onChange={event => setAnnotationKind(event.target.value as ReplayAnnotation["kind"])} className="h-9 rounded-lg border border-blue-200/[0.12] bg-[#0a1427] px-2 text-xs text-slate-200"><option value="support">Support</option><option value="resistance">Resistance</option></select><input aria-label="Annotation price" value={annotationPrice} onChange={event => setAnnotationPrice(event.target.value)} placeholder={String(current && "close" in current ? current.close : "Price")} inputMode="decimal" className="h-9 min-w-0 rounded-lg border border-blue-200/[0.12] bg-[#0a1427] px-3 text-xs text-slate-100" /><input aria-label="Annotation label" value={annotationLabel} onChange={event => setAnnotationLabel(event.target.value)} placeholder="Optional label" maxLength={120} className="h-9 min-w-0 flex-1 rounded-lg border border-blue-200/[0.12] bg-[#0a1427] px-3 text-xs text-slate-100" /><Button size="sm" disabled={createAnnotation.isPending || !Number.isFinite(Number(annotationPrice || (current && "close" in current ? current.close : NaN)))} onClick={() => { const fallbackPrice = current && "close" in current ? current.close : NaN; createAnnotation.mutate({ sessionId: session.id, kind: annotationKind, price: Number(annotationPrice || fallbackPrice), label: annotationLabel }); }} className="bg-violet-500 hover:bg-violet-400"><Plus className="mr-1 h-3.5 w-3.5" /> Add level</Button></div>{annotations.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{annotations.map(annotation => <div key={annotation.id} className={`flex items-center gap-2 rounded-lg border px-2 py-1 text-[11px] ${annotation.kind === "support" ? "border-sky-300/30 bg-sky-400/10 text-sky-100" : "border-rose-300/30 bg-rose-400/10 text-rose-100"}`}><span>{annotation.kind === "support" ? "Support" : "Resistance"} {annotation.price.toFixed(2)}{annotation.label ? ` · ${annotation.label}` : ""}</span><button type="button" aria-label={`Delete ${annotation.kind} annotation`} onClick={() => deleteAnnotation.mutate({ id: annotation.id })} className="opacity-70 transition hover:opacity-100"><Trash2 className="h-3 w-3" /></button></div>)}</div>}</div><div className="flex flex-col gap-3 border-t border-violet-300/[0.10] bg-black/[0.10] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="font-mono text-[10px] uppercase tracking-[0.13em] text-slate-500">Replay {data?.seriesType === "line" ? "price" : "candle"} · <span className="text-violet-200">{current ? replayDateLabel(current.time) : "—"}</span> · {cursor + 1}/{filteredPoints.length}</div><div className="flex items-center gap-2"><Button variant="outline" size="sm" aria-label="Previous candle" onClick={() => { setPlaying(false); setCursor(value => Math.max(0, value - 1)); }} disabled={cursor === 0} className="border-blue-200/[0.12] bg-transparent text-slate-300"><ChevronLeft className="h-4 w-4" /></Button><Button size="sm" aria-label={playing ? "Pause replay" : "Play replay"} onClick={() => setPlaying(value => !value)} className="bg-violet-500 hover:bg-violet-400">{playing ? <CirclePause className="mr-1.5 h-4 w-4" /> : <CirclePlay className="mr-1.5 h-4 w-4" />}{playing ? "Pause" : "Play"}</Button><Button variant="outline" size="sm" aria-label="Next candle" onClick={() => { setPlaying(false); setCursor(value => Math.min(filteredPoints.length - 1, value + 1)); }} disabled={cursor >= filteredPoints.length - 1} className="border-blue-200/[0.12] bg-transparent text-slate-300"><ChevronRight className="h-4 w-4" /></Button><Button variant="outline" size="sm" onClick={() => { setPlaying(false); setCursor(0); }} className="border-blue-200/[0.12] bg-transparent text-slate-300"><StepForward className="mr-1.5 h-3.5 w-3.5" /> Restart</Button></div></div></>}
  </section>;
}
